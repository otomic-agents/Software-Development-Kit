...
const doTxOut = (preBusiness: PreBusiness) => new Promise<string>(async (resolve, reject) => {
    console.log('doTxOut')

    const {txHash, uuid} = await solana.transferOutByPrivateKey(preBusiness, process.env.WALLET_KEY as string, NETWORK, RPC_SOLANA)
...
const doTxOutCfm = (preBusiness: PreBusiness, uuid: string) => new Promise<void>(async (resolve, reject) => {
    console.log('doTxOutCfm')

    const {txHash} = await solana.transferOutConfirmByPrivateKey(preBusiness, process.env.WALLET_KEY as string, NETWORK, RPC_SOLANA, uuid)
...
const doTxRefund = (preBusiness: PreBusiness, uuid: string) => new Promise<void>(async (resolve, reject) => {
    console.log('doTxOutRefund')

    const {txHash} = await solana.transferOutRefundByPrivateKey(preBusiness, process.env.WALLET_KEY as string, NETWORK, RPC_SOLANA, uuid)
...
const swap = async () => {
    ...
    const signData: {signData: SignData, signed: string} = await solana.signQuoteByPrivateKey(NETWORK, quote, process.env.WALLET_KEY as string, amount, 0, '0x50724411eb1817822e2590a43a8F0859FCc6fCD5', 
        undefined, RPC_SOLANA, RPC_BSC)
